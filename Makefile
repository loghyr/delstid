...
delstid_prot.x: $(BASEDOC)-$(VERS).txt
...
testx: delstid_prot.x
	rm -rf testx.d
	mkdir testx.d
	( \
		echo "%#include <rpc/auth.h>" > auth.x ; \
		echo "%typedef struct opaque_auth opaque_auth;" >> auth.x ; \
		cat auth.x ~/Documents/ietf/NFSv4.2/dotx.d/nfsv42.x delstid_prot.x > ff.x ; \
